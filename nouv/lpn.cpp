...
int main(){
	...
	for (int m=1;;m++){
		...
		for (int w=0;;w++){
			...
			for (int n2=min(n,max(n-m+1,n-m-int(2*log(tau)/log(2))-3));n2<=n;n2++){
				const int L=m+n2-n;
...
//					printf("P2=%d %g %f %g\n",P,probaSecret2,log(travail)/log(2),probaDetect);
					...
					if (travail<minTravail){
						minTravail=travail;
						sprintf(param,"m=%d w=%d n2=%d (L=%d : Ltau²=%f) P1=%d P2=%d : %f (%f)\n",m,w,n2,L,pow(2,L)*tau*tau,P1,P,log(travail)/log(2),probaDetect);
					}
				}
//				printf("m=%d w=%d n2=%d (L=%d) P1=%d : %f\n",m,w,n2,L,P1,log(ancTravail4)/log(2));
				if (ancTravail4>ancTravail3*8)
					break;
